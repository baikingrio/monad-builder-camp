...
1. 继续核对适合新手贡献的文档缺口；
2. 已完成一次真实 GitHub 协作：为 Moss Issue #18 提交 Windows 换行符兼容修复 PR；
3. 跟进 CI 与 Maintainer Review，并根据反馈继续修改；
4. 把已完成活动的截图和当日笔记整理为 WCB Proof。

本次真实协作 PR：[#19 fix: preserve LF line endings across checkouts](https://github.com/nishuzumi/moss/pull/19)。

### 补充学习｜Moss MCP Tools 的 Plan 审核流程

我继续阅读了 [Moss MCP Tools Reference](https://github.com/nishuzumi/moss/blob/main/docs/mcp-tools.md)，把它的四个工具整理成一个更可执行的 Dev Builder 检查顺序：
...
→ load：看参数、风险和约束
→ action：生成未签名 Plan
→ simulate：按顺序模拟并核对 effects / warnings
→ 人工核对是否符合原始意图
→ 用户钱包确认
```

这次补充学习中，我特别记录了三条不能省略的规则：

1. `action` 产生的 Plan 必须先经过 `simulate`，不能直接交给签名者；
2. 任何 warning（例如未声明资金流出、超出额度的 approval、Plan 被篡改、模拟 revert）都应停止，不应靠重新模拟“刷掉”警告；
3. 即使模拟没有 warning，也要把 `effects` 里的实际支出、收入、approval 和接收方与用户原始意图逐项对照。模拟保证的是 Plan 与声明的约束一致，不替代用户授权。

这套检查顺序可以作为我后续做 Agent 链上交互、Sponsor 或 relayer 设计时的前置审计清单。
