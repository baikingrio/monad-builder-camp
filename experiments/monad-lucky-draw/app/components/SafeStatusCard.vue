...
const emit = defineEmits<{ 'check-deployment-status': []; 'refresh-funding': [] }>()
...
const nativeMon = computed(() => formatMon(props.nativeBalanceWei))
const depositMon = computed(() => formatMon(props.entryPointDepositWei))
const tipMon = computed(() => formatMon(props.sessionTipWei))
const tipOk = computed(() => {
  try {
    return BigInt(props.nativeBalanceWei) >= BigInt(props.sessionTipWei || '0')
  } catch {
    return false
  }
})
...
<template>
  <section class="panel" aria-labelledby="safe-title">
    ...
    <div v-if="state.safeDerivationVerified && state.counterfactualSafeAddress" class="safe-address">
      ...
      <a :href="`https://testnet.monadvision.com/address/${state.counterfactualSafeAddress}`" target="_blank" rel="noopener noreferrer">在 MonadVision 查看</a>
      <dl class="balances">
        <div>
          <dt>原生余额</dt>
          <dd>{{ nativeMon }} MON</dd>
        ...
        <div>
          ...
          <dd>{{ depositMon }} MON</dd>
        </div>
      </dl>
      <p class="tip-line" :data-ok="tipOk">Roles tip 需要 ≥ {{ tipMon }} MON 原生余额</p>
      <button type="button" class="refresh-button" :disabled="!state.authenticated || fundingRefreshing" @click="emit('refresh-funding')">刷新余额</button>
    </div>
    <ul>
...
.description, li, .hint { color: #4b5563; line-height: 1.6; }.safe-state { display:flex; justify-content:space-between; align-items:center; margin:1.2rem 0 .75rem; padding: .85rem 1rem; border-radius:.7rem; background:#fff7ed; }.state-label{color:#9a3412}.safe-state strong{color:#c2410c}.warning{margin:0;padding:.75rem;border-left:3px solid #f59e0b;background:#fffbeb;color:#78350f;line-height:1.55}.check-button,.refresh-button{margin-top:.75rem;padding:.65rem .85rem;border:0;border-radius:.55rem;background:#6d28d9;color:#fff;font-weight:700;cursor:pointer}.check-button:disabled,.refresh-button:disabled{background:#9ca3af;cursor:not-allowed}.hint{margin:.5rem 0 0;font-size:.9rem}.safe-address{margin-top:.75rem;padding:.75rem;border:1px solid #ddd6fe;border-radius:.6rem;background:#f5f3ff;color:#4c1d95}.safe-address span{display:block;font-size:.8rem;font-weight:700}.safe-address code{display:block;overflow-wrap:anywhere;margin-top:.3rem;color:#312e81}.safe-address a{display:inline-block;margin-top:.5rem;color:#5b21b6;font-weight:700}.balances{margin:.75rem 0 0;display:grid;gap:.4rem}.balances div{display:flex;justify-content:space-between;gap:.75rem;font-size:.9rem}.balances dt{margin:0;color:#6d28d9}.balances dd{margin:0;font-weight:700;color:#312e81}.tip-line{margin:.55rem 0 0;font-size:.85rem;font-weight:700;color:#9a3412}.tip-line[data-ok="true"]{color:#065f46}ul{margin:1rem 0 0;padding-left:1.2rem}
</style>
