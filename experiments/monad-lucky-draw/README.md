...
> 当前状态：**首次激活和 Session Key 启用可由 Pimlico 赞助；后续 Session 抽卡必须由用户为当前 Safe 的 EntryPoint Deposit 充值支付 Gas。自建 Faucet 仍未实现。**
...
5. 用户点击「启用免弹窗抽奖」→ 浏览器生成 Session Key → **一次** Owner 授权 `addOwnerWithThreshold`（赞助）；
6. 用户 EOA 可直接向固定 EntryPoint 调用 `depositTo(currentSafe)` 充值原生 MON；之后「免弹窗抽奖」由 Session Key 本地签名、无 Paymaster，并由该 Deposit 支付 Gas（最多 3 次且受 TTL 限制）；
7. 仅在拿到公开 hash / receipt 后展示 MonadVision 链接。
...
- Session Key 作为 Safe owner（threshold=1）密码学权限较宽；仅测试网，Demo 保留短 TTL、**3 次**上限与服务端只允许 `draw()`，不提供无限抽卡；
